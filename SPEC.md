...
**Версия REPO:** [0.2.0](REPO_VERSION.md)

**Состояние:** production cutover публичного `443/tcp` завершён; все критерии
§14.3 выполнены 2026-08-20 и повторно проверены после перезапуска Xray.
...
| QR-коды | SVG генерируется локально в браузере, без внешнего API |
...
`{"status":"ok","version":"0.1.0"}` без отключения TLS-проверки; `www`
